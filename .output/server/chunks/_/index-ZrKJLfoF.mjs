...
function StreamingMonitor(props) {
  ...
  return ssr(_tmpl$$2, ssrHydrationKey(), escape(props.origin.code), escape(props.channel?.name) ?? "Select a channel", escape(props.origin.label), props.channel?.url ? ssr(_tmpl$2, ssrHydrationKey(), ssrAttribute("src", escape(props.channel.url, true), false), `pointer-events-none absolute inset-0 rounded-2xl bg-gradient-to-t from-black/85 via-black/30 to-transparent p-4 transition-opacity duration-200 ${showControls() ? "opacity-100" : "opacity-0"}`, isPlaying() ? _tmpl$3[0] + ssrHydrationKey() + _tmpl$3[1] : _tmpl$4[0] + ssrHydrationKey() + _tmpl$4[1], isMuted() || volume() === 0 ? "Muted" : "Sound", ssrAttribute("value", escape(Math.round(volume() * 100), true), false), ssrAttribute("value", escape(progressPercent(), true), false), escape(formatTime(currentTime())), escape(formatTime(duration()))) : _tmpl$5[0] + ssrHydrationKey() + _tmpl$5[1], escape(props.metrics.map((metric) => ssr(_tmpl$6, ssrHydrationKey(), escape(metric.label), escape(metric.value)))));
}
...
//# sourceMappingURL=index-ZrKJLfoF.mjs.map
